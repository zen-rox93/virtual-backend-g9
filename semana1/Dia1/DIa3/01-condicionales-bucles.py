...
# else:
#     print(f"El numero {numero} es import")
...
#si nosotros queremos iterar una coleccion de datos la mejor forma es mediante FOR
for mes in meses:
    print(mes)
# a diferencia del manejo de socpes(alcance) de la variable en JS , en python, la variable sigue existiendo fuera del FOR

for numero in range(10):
    print(numero)

#el range puede recibir hsasta 3 parametros
#range(n) > n: el limite de las iteraciones
#range(m,n) > m: el numero inicial
# range (m,n,o) > 0: en cuanto se va a incrementar o decrementar el valor en cada ciclo
...
for numero in numeros:
    if(numero > 0):
        num_positivos.append(numero) += 1
        positivos += 1
    else:
        num_negativos.append(numero) += 1
        negativos += 1

print(f"Hay {len(negativos)} negativos y {positivos} positivos")

#RPTA:
#Hay 3 negativos y son: -4, -10, -7 y hay 3 positivos y son: 7 , 8, ,25
...
# dadao los siguiente numeros:
numeros = [1, 2, 5, 9, 12, 15, 17, 19, 21, 39]
#indicar cuantos de ellos son multiplos de 3 y 5, ademas si hay un multiplo de 3 y de 5 no contabilizarlos
...
print(f"Multos de {multiplos_3}, multiplos de 5 {multiplos_5}")
