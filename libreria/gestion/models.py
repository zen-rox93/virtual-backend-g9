...
class ProductoModel(models.Model):
    ...
    class OpcionesUM(models.TextChoices):
        ...
        CIENTO = 'CI', 'CIENTO'
        MILLAR = 'MI', 'MILLAR'
#tipos de datos del ORM => https://docs.djangoproject.com/en/3.2/ref/models/fields/
...
# Parametros genericos de los tipos de datos =>https://docs.djangoproject.com/en/3.2/ref/models/fields/#field-options
    ...
    class Meta:
        ...
        # si queremos que sea sola ascente se pondra solamente el nombre del atributo, si queremos que sea Descendente antes del atributo prondremos un guion (-)
        ordering = ['-productoPrecio']
        #verbose_name y verbonse_name_plural => sirve para el planel administrativo (CMS) de django
...
class CabeceraModel(models.Model):
    ...
    #PROTECT => no permite la eliminacion de clientes siempre  y cuando tengan una cabecera
    #SET_NULL => elimina el cliente y todas sus cabeceras la cambia el valor a null
    #DO_NOTHING => no hacew nada, mantiene su valor a pesar que se elimino dicho cliente
