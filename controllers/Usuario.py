...
class RegistroController(Resource):
    ...
    def post(self):
        ...
        if search(PATRON_PASSWORD, password) is None:
            return {
                "message": "Password incorrecto, minimo 6 caracteres una mayuscuka, una minuscula y un simbolo especial"
            }, 400
        try:
            ...
            #hashpw que lo que hara sera combinar nuestra pwd con el salt generado prviamente
...
class LoginController(Resource):
    serialiazador = reqparse.RequestParser(bundle_errors=True)
    serialiazador.add_argument(
        'correo',
        type=str,
        required=True,
        location='json',
    ...
    def post(self):
        #si el usuario existe en la bd indicarlo en un mensaje indicar que no existe y un estado 404
        data = self.serialiazador.parse_args()
...
class ResetearPasswordController(Resource):
    ...
    def post(self):
        ...
        Has solicitado el reinicio de tu contraseña, haz click en el siguiente link para efectuarla:
            <br>
            <a href="{}"<button style="background-color:peru; color:white; border:none; " >Cambiar</a></button>
