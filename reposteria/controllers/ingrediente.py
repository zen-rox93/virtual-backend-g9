...
# serializador => elemento que convierte los parametros que me envia el front para tener un uso correcto con el backend

serializador = reqparse.RequestParser()
serializador.add_argument(
    'nombre', # nombre del argumento que esperar ser recibido
...
class IngredienteController(Resource):
    def get(self, id):
        ...
        # Devolver el ingrediente selecionado y si no existe devolver un mensaje que diga que el ingrediente no fue encontrado con un estado 404 (not found)
        if resultado:
            data = resultado.__dict__
            # https://docs.sqlalchemy.org/en/14/orm/internals.html#sqlalchemy.orm.InstanceState
            # devuelve la instancia de la cual se esta extrayendo la info del registro de la base de datos
...
serializadorFiltro.add_argument(
    'nombre',
    location='args', # tiene que ser enviado por los querystrings http://127.0.0.1/ruta?nombre=valor&valor2
    required=False,
    type=str
)
...
class FiltroIngredientesController(Resource):
    ...
    #     }
        ...
        #A diferencia de hacer solamente un .all() sin un with_entitis esto nos retornara un lista de objetos de la clase Row https://docs.sqlalchemy.org/en/14/orm/internals.html#sqlalchemy.orm.InstanceState
