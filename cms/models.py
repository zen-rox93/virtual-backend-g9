...
class PlatoModel(models.Model):
    ...
    # a diferencia del FileField el imageFiel solamente permitira el guardado de archivos con extesion de imagenes (.jpg, .png , .svg, .jpeg)
    ...
    #se actualizara su valor cunado el registro sufra alguna modificacion
    # auto_now = agarrara la fecha actual cuando parte del registro o en su totalidad sea modifcada
...
class UsuarioModel(AbstractBaseUser, PermissionsMixin):
    # si queremos modificar todo el auth model entonces tendremos que usar el AbstractBaseUser, si queremos solamente modificar first_name, last_name, entonces usaremos el AbstractUser
    # PermissionsMixion = es la clase encargada de dar todos los permisos a nivel administrativo
    ...
    usuarioApellido = models.CharField(max_lenght=50, db_column='apellido', verbose_name='Apellido del usuario')
    ...
    # ahora agregamos el comportamiento cuando se llame el createsuperuser y al create user
...
class DetallePedidoModel(models.Model):
    ...
    detalleCantidad = models.IntegerField(
        db_column='cantidad', null=False)

    detalleSubtotal = models.DecimalField(max_digits=5, decimal_places=2, db_column='sub_total')

    plato = models.ForeignKey(
        to=PlatoModel, related_name='platoDetalle', db_column='plato_id')
    pedido = models.ForeignObject(
